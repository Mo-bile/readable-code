...
public class MinesweeperGame {
    ...
    private static int getSelectedRowIndex(String cellInput) {
        char cellInputRow = cellInput.charAt(1); // 1줄이라도 의미있음 -> 의미로서
        return convertRowFrom(cellInputRow);
    }

    private static int getSelectedColIndex(String cellInput) {
        char cellInputCol = cellInput.charAt(0); // 전치사와 매개변수와 자연스럽게 이어지게 활용 가능
    ...
    private static void initinalizeGame() {
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                BOARD[row][col] = CLOSE_CELL_SIGN;
            }
        }
        for (int i = 0; i < LAND_MINE_COUNT; i++) { // 여기 10은 지뢰갯수임
